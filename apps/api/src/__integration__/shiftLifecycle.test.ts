...
test('the office sees the shift in the timesheet queue and completes it', async () => {
  ...
  assert.equal(Number(completed.totalEstimated), 175.75, '9.5h at 18.50');
});
...
test('a short shift is invoiced at the four-hour minimum', async () => {
  ...
  // The money is floored at four hours: 4 x 18.50.
  assert.equal(Number(row.totalEstimated), 74);
