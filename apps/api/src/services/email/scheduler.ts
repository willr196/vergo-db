...
function buildScheduledEmailHtml(emailType: ScheduledEmailType, data: Record<string, any>): string {
  const templateData: EmailTemplateData = { ...data };
  switch (emailType) {
    case 'quote-followup':
      return templates.quoteFollowupEmail({
        ...templateData,
        recipientName: templateData.recipientName ?? templateData.name,
      });
    case 'application-review-reminder':
      return templates.applicationReviewReminderEmail(templateData);
    case 'shift-reminder':
      return templates.shiftReminderEmail({
        ...templateData,
        recipientName: templateData.recipientName ?? templateData.name,
        jobLocation: templateData.jobLocation ?? (data.location as string | undefined),
      });
    default:
      // Exhaustive guard for future additions.
      throw new Error(`Unsupported scheduled email type: ${emailType}`);
  }
}
...
export async function scheduleEmail(options: ScheduleEmailOptions): Promise<string | null> {
  ...
  const html = buildScheduledEmailHtml(emailType, data);
