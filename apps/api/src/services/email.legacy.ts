        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="padding: 30px; background: #f9f9f9;">
            ...
            <div style="background: white; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
              <p><strong>From:</strong> ${safe(data.name)}</p>
              <p><strong>Email:</strong> <a href="mailto:${safe(data.email)}">${safe(data.email)}</a></p>
              ${data.phone ? `<p><strong>Phone:</strong> ${safe(data.phone)}</p>` : ''}
              <p><strong>Company:</strong> ${safe(data.company)}</p>
              ${data.date ? `<p><strong>Event Date:</strong> ${safe(data.date)}</p>` : '<p><strong>Event Date:</strong> TBC / Flexible</p>'}
              <p><strong>Staff Needed:</strong> ${safe(data.staffCount)}</p>
            </div>
            
            <div style="background: white; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
              <h3 style="margin-top: 0; color: #2c3e2f;">Roles Required:</h3>
              <ul style="margin: 0; padding-left: 20px;">
                ${data.roles.map(role => `<li style="margin-bottom: 8px;">${safe(role)}</li>`).join('')}
              </ul>
            </div>
          ...
          <div style="padding: 20px; text-align: center; color: #666; font-size: 12px;">
            <p>VERGO Ltd | London, United Kingdom</p>
          </div>
        </div>
      `
    });
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="padding: 30px; background: #f9f9f9;">
            ...
            <div style="background: white; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
              <p><strong>Applicant:</strong> ${safe(data.applicantName)}</p>
              <p><strong>Email:</strong> <a href="mailto:${safe(data.email)}">${safe(data.email)}</a></p>
              ${data.phone ? `<p><strong>Phone:</strong> ${safe(data.phone)}</p>` : ''}
            </div>
            ...
            ${data.cvOriginalName ? `
              <div style="background: white; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
                <p style="margin: 0;"><strong>CV:</strong> ${safe(data.cvOriginalName)}</p>
              </div>
            ` : ''}
            ...
            <div style="margin-top: 30px; padding: 20px; background: #e8f4fd; border-left: 4px solid #0066cc; border-radius: 4px;">
              <p style="margin: 0; font-size: 12px; color: #333;">
                <strong>Application ID:</strong> ${safe(data.applicationId)}
              </p>
            </div>
          </div>
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="padding: 30px; background: #f9f9f9;">
            ...
            <p>Thank you for applying to join our team at VERGO!</p>
            ...
            <div style="background: white; padding: 20px; border-radius: 8px; margin: 20px 0;">
              ...
              <p style="margin-bottom: 0; font-size: 12px; color: #666;">
                Reference: ${safe(data.applicationId)}
              </p>
            </div>
            ...
            <p>We appreciate your interest in VERGO. If you have any questions, feel free to reply to this email.</p>
            
            <p style="margin-bottom: 0;">Best regards,<br>
            <strong>The VERGO Team</strong></p>
          </div>
          ...
          <div style="padding: 20px; text-align: center; color: #666; font-size: 12px; background: #f0f0f0;">
            ...
            <p style="margin: 0;">
              <a href="${safe(env.webOrigin)}" style="color: #D4AF37; text-decoration: none;">www.vergoltd.com</a>
            </p>
          </div>
        </div>
      `
...
export async function sendUserVerificationEmail(data: {
  ...
  try {
    const verifyUrl = `${env.webOrigin}/api/v1/user/verify-email?token=${encodeURIComponent(data.token)}`;
    const safeVerifyUrl = safe(verifyUrl);
    ...
    const result = await resend.emails.send({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="padding: 30px; background: #f9f9f9;">
            <h2 style="color: #2c3e2f; margin-top: 0;">Welcome, ${safe(data.name)}!</h2>
            
            <p>Thanks for creating an account with VERGO. Please verify your email address to get started.</p>
            
            <div style="text-align: center; margin: 30px 0;">
              <a href="${safeVerifyUrl}" style="display: inline-block; padding: 15px 40px; background: #D4AF37; color: white; text-decoration: none; border-radius: 8px; font-weight: bold; font-size: 16px;">
                Verify Email Address
              </a>
            </div>
            
            <p style="color: #666; font-size: 14px;">Or copy this link into your browser:</p>
            <p style="color: #666; font-size: 12px; word-break: break-all; background: #fff; padding: 10px; border-radius: 4px;">${safeVerifyUrl}</p>
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="padding: 30px; background: #f9f9f9;">
            ...
            <div style="text-align: center; margin: 30px 0;">
              <a href="${safeResetUrl}" style="display: inline-block; padding: 15px 40px; background: #D4AF37; color: white; text-decoration: none; border-radius: 8px; font-weight: bold; font-size: 16px;">
                Reset Password
              </a>
            </div>
            
            <p style="color: #666; font-size: 14px;">Or copy this link into your browser:</p>
            <p style="color: #666; font-size: 12px; word-break: break-all; background: #fff; padding: 10px; border-radius: 4px;">${safeResetUrl}</p>
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="padding: 30px; background: #f9f9f9;">
          ...
          <div style="background: white; padding: 20px; border-radius: 8px;">
            <p><strong>Job:</strong> ${safe(data.jobTitle)}</p>
            <p><strong>Applicant:</strong> ${safe(data.applicantName)}</p>
      ...
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="padding: 30px; background: #f9f9f9;">
          ...
          <div style="background: white; padding: 20px; border-radius: 8px; border-left: 4px solid #D4AF37; margin: 20px 0;">
            ...
            <p style="margin: 5px 0 0; color: #666;">📍 ${safe(data.location)} • 📅 ${safe(dateStr)}</p>
          </div>
...
export async function sendClientVerificationEmail(data: {
  ...
  try {
    const verifyUrl = `${env.webOrigin}/api/v1/clients/verify-email?token=${encodeURIComponent(data.token)}`;
    const safeVerifyUrl = safe(verifyUrl);
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="padding: 30px; background: #f9f9f9;">
            ...
            <div style="text-align: center; margin: 30px 0;">
              <a href="${safeLoginUrl}" style="display: inline-block; padding: 15px 40px; background: #D4AF37; color: white; text-decoration: none; border-radius: 8px; font-weight: bold; font-size: 16px;">
                Log In to Dashboard
              </a>
            </div>
          ...
          <div style="padding: 30px; background: #f9f9f9;">
            ...
            ${data.reason ? `
              <div style="background: #fff; padding: 20px; border-radius: 8px; border-left: 4px solid #666; margin: 20px 0;">
                <p style="margin: 0; color: #333;"><strong>Reason:</strong> ${safe(data.reason)}</p>
              </div>
            ` : ''}
